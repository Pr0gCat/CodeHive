...
describe('TaskEventEmitter', () => {
  ...
  it('should emit and receive phase updated events', (done) => {
    ...
    taskEventEmitter.emitPhaseUpdated(taskId, phaseId, progress, message, details);
  });
  ...
  it('should only receive events for subscribed task', (done) => {
    ...
    const unsubscribe = taskEventEmitter.onTaskEvent(taskId1, (event: TaskEventData) => {
  ...
  it('should handle multiple subscribers for the same task', (done) => {
    ...
    const unsubscribe1 = taskEventEmitter.onTaskEvent(taskId, (event: TaskEventData) => {
      expect(event.taskId).toBe(taskId);
      receivedCount++;
    ...
    const unsubscribe2 = taskEventEmitter.onTaskEvent(taskId, (event: TaskEventData) => {
      expect(event.taskId).toBe(taskId);
      receivedCount++;
