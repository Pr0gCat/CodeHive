...
describe('BranchManager', () => {
  ...
  describe('createFeatureBranch', () => {
    it('should create and switch to a new feature branch', async () => {
      ...
      const result = await branchManager.createFeatureBranch('test-id', 'Test Feature')

      expect(result.success).toBe(true)
      expect(result.output).toContain('Created and switched to branch: feature/cycle-test-id-test-feature')
    })
    ...
    it('should sanitize feature names for branch creation', async () => {
      mockExec('Switched to branch feature/cycle-test-id-test-feature-with-spaces')

      const result = await branchManager.createFeatureBranch('test-id', 'Test Feature With Spaces')

      expect(result.success).toBe(true)
      expect(result.output).toContain('feature/cycle-test-id-test-feature-with-spaces')
    })
  })
  ...
  describe('createCheckpointBranch', () => {
    it('should create a checkpoint branch', async () => {
      ...
      const result = await branchManager.createCheckpointBranch('test-id', 'red')

      expect(result.success).toBe(true)
      expect(result.output).toContain('Created checkpoint branch: checkpoint/red-phase-start')
    })
  ...
  describe('rollbackToCheckpoint', () => {
    it('should rollback to specified checkpoint', async () => {
      ...
      const result = await branchManager.rollbackToCheckpoint('checkpoint/red-phase-start')

      expect(result.success).toBe(true)
      expect(result.output).toContain('Rolled back to checkpoint: checkpoint/red-phase-start')
    })
    ...
    it('should handle rollback failures', async () => {
      ...
      const result = await branchManager.rollbackToCheckpoint('checkpoint/invalid')
  ...
  describe('acquireLock', () => {
    it('should acquire lock when branch is not locked', async () => {
      const result = await branchManager.acquireLock('cycle-id', 'feature-branch', 'test-agent')
    ...
    it('should reject lock when branch is already locked', async () => {
      ...
      const firstLock = await branchManager.acquireLock('cycle-id', 'feature-branch', 'agent1')
      expect(firstLock).toBe(true)

      // Second lock should fail
      const secondLock = await branchManager.acquireLock('cycle-id', 'feature-branch', 'agent2')
  ...
  describe('releaseLock', () => {
    it('should release lock successfully', async () => {
      await branchManager.acquireLock('cycle-id', 'feature-branch', 'test-agent')
      await branchManager.releaseLock('feature-branch')

      // Should be able to acquire lock again
      const result = await branchManager.acquireLock('cycle-id', 'feature-branch', 'other-agent')
      expect(result).toBe(true)
    })
  })
  ...
  describe('switchToBranch', () => {
    it('should switch to branch successfully', async () => {
      ...
      const result = await branchManager.switchToBranch('cycle-id', 'feature-branch')
    ...
    it('should respect branch locks', async () => {
      await branchManager.acquireLock('cycle-id', 'locked-branch', 'test-agent')

      const result = await branchManager.switchToBranch('other-cycle-id', 'locked-branch')
    ...
    it('should handle git checkout failures', async () => {
      ...
      const result = await branchManager.switchToBranch('cycle-id', 'nonexistent-branch')
