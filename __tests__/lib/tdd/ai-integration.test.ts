...
describe('AITDDIntegration', () => {
  ...
  describe('generateImplementationCode', () => {
    ...
    it('should generate implementation code using AI', async () => {
      ...
      mockExecutor.execute.mockResolvedValue({
        success: true,
        output:
          'export function login(credentials) { return validateUser(credentials); }',
      });
      ...
      const result =
        await aiIntegration.generateImplementationCode(mockRequest);
    ...
    it('should handle implementation failures with decision query', async () => {
      ...
      await expect(
        aiIntegration.generateImplementationCode(mockRequest)
    ...
    const mockArtifacts = [
      {
        ...mockArtifact,
        content:
          'function veryLongFunctionNameThatExceedsReasonableLimitsAndShouldBeRefactoredForBetterReadabilityAndMaintainability() { /* very long code without error handling */ }',
      },
      {
        ...mockArtifact,
        id: 'artifact-2',
        content:
          'function shortFunction() { try { return result; } catch (e) { throw e; } }',
      },
    ];

    it('should detect code quality issues', async () => {
      const queries = await aiIntegration.analyzeCodeQuality(
        'test-cycle-id',
        mockArtifacts
      );
      ...
      const goodArtifacts = [
        {
          ...mockArtifact,
          content:
            'function goodFunction() { try { return validateInput(); } catch (error) { handleError(error); } }',
        },
      ];
  ...
  describe('buildTestGenerationPrompt', () => {
    it('should build comprehensive prompt for test generation', async () => {
      ...
      const prompt = aiIntegration['buildTestGenerationPrompt'](
        criterion,
        context as any
      );
  ...
  describe('buildImplementationPrompt', () => {
    it('should build prompt with test code context', async () => {
      ...
      const prompt = aiIntegration['buildImplementationPrompt'](
        test as any,
        context as any
      );
  ...
  describe('performCodeQualityChecks', () => {
    it('should detect various code quality issues', async () => {
      ...
      expect(issues).toContain(
        'File is too large, consider splitting into smaller modules'
      );
