...
export async function GET(request: NextRequest) {
  try {
    ...
    tokenUsage.forEach(usage => {
      ...
      projectUsageMap.set(
        projectName,
        projectUsageMap.get(projectName) +
          usage.inputTokens +
          usage.outputTokens
      );
    ...
    const byProject = Array.from(projectUsageMap.entries())
      .map(([name, used]) => ({
        ...
        used,
        percentage:
          totalTokens > 0 ? Math.round((used / totalTokens) * 100) : 0,
      }))
    ...
    const previousAvg =
      previousUsage.reduce((sum, day) => sum + day.used, 0) / 7;
    const weeklyChange =
    ...
    const trends = {
      ...
      monthlyAverage: Math.round(
        dailyUsage.reduce((sum, day) => sum + day.used, 0) / dailyUsage.length
      ),
    ...
    const cycleTimes = projects.map(project => {
      const completedCycles = project.cycles.filter(
      ...
      const avgTime =
        completedCycles.length > 0
          ? completedCycles.reduce((sum, cycle) => {
              const duration =
                cycle.updatedAt.getTime() - cycle.createdAt.getTime();
    ...
    const bottlenecks = projects
      .filter(project => {
        const stuckCycles = project.cycles.filter(
          cycle =>
            cycle.status !== 'COMPLETED' &&
            new Date().getTime() - cycle.createdAt.getTime() >
              24 * 60 * 60 * 1000
        );
    ...
    const successRates = Array.from(agentPerformanceMap.entries()).map(
      ([agent, stats]) => ({
        agent,
        rate:
          stats.total > 0
    ...
    const usagePatterns = Array.from(agentPerformanceMap.entries()).map(
      ([agent, stats]) => ({
        agent,
        hours:
          stats.completedTasks.length > 0
            ? Math.round(
                (stats.totalHours / stats.completedTasks.length) * 10
              ) / 10
            : 0,
        efficiency:
          stats.total > 0
    ...
    const timeMetrics = {
      ...
      responseTime: (() => {
        const completedTasksWithTiming = agentTasks.filter(
          task =>
        ...
        const responseTimes = completedTasksWithTiming
          .map(
            task =>
