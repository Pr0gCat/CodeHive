...
export async function GET(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      console.log(
        `🔗 TDD Cycles SSE connection established for project: ${projectId}`
      );
      ...
      const cleanup = () => {
        clearInterval(pollInterval);
        console.log(
          `🔌 TDD Cycles SSE connection closed for project: ${projectId}`
        );
      };
    ...
    cancel() {
      console.log(
        `🔌 TDD Cycles SSE stream cancelled for project: ${projectId}`
      );
    },
  });
