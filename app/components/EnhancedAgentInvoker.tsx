...
export default function EnhancedAgentInvoker({ cardId, projectId, onClose }: EnhancedAgentInvokerProps) {
  ...
  const fetchAgentCapabilities = async (agentType: string) => {
    try {
      const response = await fetch(`/api/agents/capabilities?agentType=${agentType}&projectId=${projectId}`);
      const data = await response.json();
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
          <div className="text-center">
            ...
            <h3 className="text-lg font-semibold text-primary-100 mb-2">Agent Task Executed</h3>
            <p className="text-primary-600 mb-4">
              Your agent task has been {selectedAgent === 'project-manager' ? 'orchestrated' : 'queued'} successfully.
            </p>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-primary-100">Execute Agent</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {agentCapabilities && (
            <div className="bg-primary-950 p-4 rounded-lg">
              <h4 className="font-medium text-primary-100 mb-2">Agent Capabilities</h4>
          ...
          <div>
            ...
            {validating && (
              <div className="mt-1 text-sm text-primary-500 flex items-center">
                <svg className="animate-spin -ml-1 mr-2 h-3 w-3 text-primary-500" fill="none" viewBox="0 0 24 24">
            ...
            {validation && !validating && (
              <div className={`mt-1 text-sm ${validation.valid ? 'text-accent-600' : 'text-red-600'}`}>
                {validation.valid ? (
          ...
          {agentCapabilities && agentCapabilities.supportedCommands.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-primary-300 mb-2">
                Supported Commands
              </label>
              <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto">
                {agentCapabilities.supportedCommands.map((cmd, index) => (
                  <div key={index} className="border border-primary-800 rounded p-3">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h5 className="font-medium text-primary-100 text-sm">{cmd.name}</h5>
                        <p className="text-xs text-primary-600 mt-1">{cmd.description}</p>
                      </div>
                    </div>
                    <div className="mt-2">
                      <p className="text-xs font-medium text-primary-300 mb-1">Examples:</p>
                      <div className="space-y-1">
                        {cmd.examples.slice(0, 2).map((example, exampleIndex) => (
                          <button
                            key={exampleIndex}
                            type="button"
