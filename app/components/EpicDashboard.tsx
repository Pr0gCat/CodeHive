...
export default function EpicDashboard({ projectId }: EpicDashboardProps) {
  ...
  const [selectedEpic, setSelectedEpic] = useState<EpicWithProgress | null>(null);
  ...
  return (
    <div className="bg-primary-800 rounded-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-accent-50">Epic 管理儀表板</h2>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-primary-700 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-accent-50">{epics.length}</div>
            </div>
            <div className="bg-primary-700 rounded-lg p-4">
            ...
            <div className="bg-primary-700 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-accent-50">
                {epics.length > 0 
                  ? Math.round((epics.filter(e => e.phase === 'DONE').length / epics.length) * 100)
                  : 0}%
              </div>
            </div>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getPhaseColor(epic.phase)}`}>
                          {epic.phase}
                        </span>
                    ...
                    {epic.description && (
                      <div className="text-sm text-primary-300 mb-2">{epic.description}</div>
                    )}
                    {epic.coreValue && (
                  ...
                  <div className="text-right">
                    <div className="text-sm font-medium text-accent-50">
                      {epic.progress.storiesCompleted}/{epic.progress.storiesTotal} Stories
                    </div>
                    <div className="text-xs text-primary-400">
                      {epic.progress.storyPointsCompleted}/{epic.progress.storyPointsTotal} SP
                    </div>
                  </div>
                </div>
                ...
                <div className="mb-3">
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-accent-50">{epic.progress.percentage}%</span>
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-primary-600">
                    <div className="flex items-center gap-4 text-xs">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-primary-400">{epic.stories.filter(s => s.status === 'TODO').length} Todo</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 bg-amber-500 rounded-full"></div>
                        <span className="text-primary-400">{epic.stories.filter(s => s.status === 'IN_PROGRESS').length} 進行中</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                        <span className="text-primary-400">{epic.stories.filter(s => s.status === 'DONE').length} 完成</span>
                      </div>
                    </div>
                  </div>
                )}
