...
export default function TDDDashboard({ projectId }: TDDDashboardProps) {
  ...
  const getPhaseColor = (phase: string) => {
    switch (phase) {
      case 'RED': return 'text-red-500 bg-red-900';
      case 'GREEN': return 'text-green-500 bg-green-900';
      case 'REFACTOR': return 'text-blue-500 bg-blue-900';
      case 'REVIEW': return 'text-purple-500 bg-purple-900';
  ...
  return (
    <div className="bg-primary-800 rounded-lg p-6">
      <h2 className="text-xl font-semibold text-accent-50 mb-6">TDD 開發儀表板</h2>
      ...
      {activeCycle ? (
        <div className="space-y-6">
          ...
          <div className="bg-primary-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-medium text-accent-50">{activeCycle.title}</h3>
                {activeCycle.description && (
                  <p className="text-sm text-primary-300 mt-1">{activeCycle.description}</p>
                )}
              </div>
              <div className={`flex items-center gap-2 px-3 py-1 rounded-full ${getPhaseColor(activeCycle.phase)}`}>
                ...
                {['RED', 'GREEN', 'REFACTOR', 'REVIEW'].map((phase, index) => (
                  <div
                    ...
                    className={`flex-1 h-2 rounded-full transition-colors ${
                      index <= ['RED', 'GREEN', 'REFACTOR', 'REVIEW'].indexOf(activeCycle.phase)
                        ? getPhaseColor(phase).split(' ')[0].replace('text', 'bg')
                        : 'bg-primary-600'
                    }`}
                  />
                ))}
            ...
            {/* Tests Status */}
            <div className="mb-4">
              <h4 className="text-sm font-medium text-accent-50 mb-2">測試狀態</h4>
                ...
                {activeCycle.tests.length > 0 ? (
                  activeCycle.tests.map((test) => (
                    <div key={test.id} className="flex items-center justify-between text-sm">
                      ...
                      {test.duration && (
                        <span className="text-primary-400">{test.duration}ms</span>
                      )}
                    </div>
                  ))
                ) : (
          ...
          <div className="bg-primary-700 rounded-lg p-4">
            <h4 className="text-sm font-medium text-accent-50 mb-3">最近生成的檔案</h4>
            <div className="space-y-2">
                ...
                <div key={artifact.id} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-1 rounded text-xs ${
                      artifact.type === 'CODE' ? 'bg-blue-900 text-blue-300' :
                      artifact.type === 'TEST' ? 'bg-green-900 text-green-300' :
        ...
        <div className="text-center py-8">
          <p className="text-primary-400 mb-4">目前沒有活躍的 TDD 週期</p>
          <button className="px-4 py-2 bg-accent-600 text-accent-50 rounded-lg hover:bg-accent-700">
            開始新的 TDD 週期
          </button>
        </div>
      )}
      ...
      {cycles.filter(c => c.status === 'COMPLETED').length > 0 && (
        <div className="mt-6">
          <h3 className="text-sm font-medium text-accent-50 mb-3">已完成的週期</h3>
          <div className="space-y-2">
            {cycles.filter(c => c.status === 'COMPLETED').map((cycle) => (
              <div key={cycle.id} className="bg-primary-700 rounded p-3 flex items-center justify-between">
                <div>
