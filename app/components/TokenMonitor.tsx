  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-primary-900 rounded-lg p-6 border border-primary-700">
        ...
        {/* Global Usage Status */}
        <div className={`p-4 rounded-lg border mb-6 ${getStatusColor(global.status)}`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center">
              <span className="text-2xl mr-3">{getStatusIcon(global.status)}</span>
              <div>
                <div className="font-semibold">
                  {global.status === 'critical' ? '危險' : 
                   global.status === 'warning' ? '警告' : '正常'}
                </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-primary-800 rounded-lg p-4 border border-primary-600">
            <div className="text-2xl font-bold text-accent-50">{summary.totalProjects}</div>
            <div className="text-sm text-primary-300">專案總數</div>
          </div>
          <div className="bg-primary-800 rounded-lg p-4 border border-primary-600">
            <div className="text-2xl font-bold text-red-400">{summary.projectsOverBudget}</div>
            <div className="text-sm text-primary-300">超出預算專案</div>
          </div>
          <div className="bg-primary-800 rounded-lg p-4 border border-primary-600">
            <div className="text-2xl font-bold text-yellow-400">{summary.projectsNearLimit}</div>
            <div className="text-sm text-primary-300">接近上限</div>
          </div>
          <div className="bg-primary-800 rounded-lg p-4 border border-primary-600">
            <div className="text-2xl font-bold text-accent-50">{summary.averageUsage.toFixed(1)}%</div>
      ...
      {projects.length > 0 && (
        <div className="bg-primary-900 rounded-lg p-6 border border-primary-700">
          <h3 className="text-lg font-semibold text-accent-50 mb-4">專案預算狀況</h3>
          <div className="space-y-3">
            {projects.slice(0, 10).map((project) => (
              <div key={project.id} className="flex items-center justify-between bg-primary-800 rounded-lg p-4 border border-primary-600">
                <div className="flex items-center flex-1">
                  <span className="text-lg mr-3">{getStatusIcon(project.status)}</span>
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-medium text-accent-50">{project.name}</h4>
                      <div className="text-sm text-primary-400">
                        {project.allocatedPercentage > 0 
                          ? `${(project.allocatedPercentage * 100).toFixed(1)}% 已分配`
                          : '尚未分配預算'
                        }
                      </div>
                    </div>
                    ...
                    <div className="w-full bg-primary-700 rounded-full h-2 mb-2">
                      <div 
                        className={`h-2 rounded-full transition-all duration-300 ${
                          project.isOverBudget ? 'bg-red-500' : getProgressBarColor(project.status)
                        }`}
                    ...
                    <div className="flex justify-between text-xs text-primary-400">
                      <span>
                        已用：{formatShortNumber(project.usedTokens)}
                        {project.isOverBudget && (
                          <span className="text-red-400 ml-1">（超出預算）</span>
                        )}
                      </span>
                      <span>
                        預算：{project.budgetTokens > 0 
                          ? formatShortNumber(project.budgetTokens) 
                          : '尚未設定'}
                      </span>
                    </div>
                  </div>
                ...
                <div className="text-right ml-4">
                  <div className={`text-lg font-semibold ${
                    project.isOverBudget ? 'text-red-400' : 
                    project.status === 'warning' ? 'text-yellow-400' : 'text-green-400'
                  }`}>
