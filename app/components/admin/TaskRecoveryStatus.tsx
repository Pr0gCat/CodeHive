    ...
    <div className="bg-primary-900 border border-primary-800 rounded-lg p-6">
      ...
      {data && (
        <div className="space-y-6">
          {/* System Status */}
          <div>
            <h3 className="text-lg font-medium text-accent-50 mb-3">系統狀態</h3>
            ...
            <div className="mb-4 p-3 rounded-lg border flex items-center space-x-3 ${data.needsRecovery ? 'bg-yellow-900/20 border-yellow-700' : 'bg-green-900/20 border-green-700'}">
              {data.needsRecovery ? (
                <>
                  ...
                  <div>
                    <span className="text-yellow-400 font-medium">需要恢復</span>
                    <p className="text-yellow-300 text-sm">
                      發現 {data.systemStatus.projects.initializing} 個初始化中的專案或 {data.systemStatus.tasks.pending} 個待處理任務
                    </p>
                  </div>
                </>
              ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-primary-800 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-accent-50">{data.systemStatus.projects.total}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-yellow-400">初始化中</span>
                    <span className="text-yellow-400">{data.systemStatus.projects.initializing}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-green-400">活躍</span>
                    <span className="text-green-400">{data.systemStatus.projects.active}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">已歸檔</span>
                    <span className="text-gray-400">{data.systemStatus.projects.archived}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-primary-800 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    <span className="text-primary-300">總計</span>
                    <span className="text-accent-50">{data.systemStatus.tasks.total}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-yellow-400">待處理</span>
                    <span className="text-yellow-400">{data.systemStatus.tasks.pending}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-blue-400">執行中</span>
                    <span className="text-blue-400">{data.systemStatus.tasks.running}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-green-400">已完成</span>
                    <span className="text-green-400">{data.systemStatus.tasks.completed}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-red-400">失敗</span>
                    <span className="text-red-400">{data.systemStatus.tasks.failed}</span>
                  </div>
                </div>
              </div>
            </div>
