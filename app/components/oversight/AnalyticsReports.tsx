...
export function AnalyticsReports() {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await fetch(`/api/oversight/analytics?range=${timeRange}`);
      if (response.ok) {
  ...
  const exportReport = async (format: 'csv' | 'pdf') => {
    try {
      const response = await fetch(`/api/oversight/analytics/export?format=${format}&range=${timeRange}`);
      if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div>
            <h2 className="text-xl font-semibold text-accent-50 mb-2">分析報告</h2>
      ...
      {activeMetric === 'tokens' && analyticsData?.tokenUsage && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-primary-300">週變化</h3>
                  <p className={`text-2xl font-bold ${getChangeColor(analyticsData.tokenUsage.trends.weeklyChange)}`}>
                    {getChangeIcon(analyticsData.tokenUsage.trends.weeklyChange)} {Math.abs(analyticsData.tokenUsage.trends.weeklyChange)}%
                  </p>
                </div>
            ...
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-primary-300">月平均</h3>
            ...
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-medium text-primary-300">峰值使用</h3>
                  <p className="text-2xl font-bold text-accent-50">
          ...
          {/* Daily Usage Chart */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">每日 Token 使用量</h3>
          ...
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">專案 Token 使用分佈</h3>
            <div className="space-y-4">
              {analyticsData.tokenUsage.byProject.slice(0, 10).map((project) => (
                <div key={project.name} className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="text-sm text-accent-50">{project.name}</span>
      ...
      {activeMetric === 'performance' && analyticsData?.projectPerformance && (
        <div className="space-y-6">
          {/* Completion Rates */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">Epic 完成率</h3>
            <div className="space-y-4">
              {analyticsData.projectPerformance.completionRates.map((project) => (
                <div key={project.project} className="flex items-center justify-between">
                  <div className="flex-1">
                    <div className="flex justify-between mb-1">
                      <span className="text-sm text-accent-50">{project.project}</span>
                      <span className="text-sm text-primary-300">
                    ...
                    <div className="w-full bg-primary-700 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full ${
                          project.rate >= 80 ? 'bg-green-500' :
                          project.rate >= 60 ? 'bg-yellow-500' : 'bg-red-500'
                        }`}
          ...
          {/* Cycle Times */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">平均循環時間</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {analyticsData.projectPerformance.cycleTimes.map((project) => (
                <div key={project.project} className="bg-primary-800 rounded-lg p-4">
                  <h4 className="font-medium text-accent-50 mb-2">{project.project}</h4>
          ...
          {/* Bottlenecks */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">瓶頸分析</h3>
            <div className="space-y-3">
              {analyticsData.projectPerformance.bottlenecks.map((bottleneck, index) => (
                <div key={index} className="flex items-start gap-3 p-4 bg-yellow-900 border border-yellow-700 rounded-lg">
                  <AlertCircle className="h-5 w-5 text-yellow-400 mt-0.5 flex-shrink-0" />
                  <div className="flex-1">
                    <h4 className="font-medium text-yellow-100">{bottleneck.project}</h4>
                    <p className="text-sm text-yellow-200 mt-1">{bottleneck.issue}</p>
      ...
      {activeMetric === 'agents' && analyticsData?.agentPerformance && (
        <div className="space-y-6">
          {/* Success Rates */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">AI 代理成功率</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {analyticsData.agentPerformance.successRates.map((agent) => (
                <div key={agent.agent} className="bg-primary-800 rounded-lg p-6 text-center">
                  <h4 className="font-medium text-accent-50 mb-3">{agent.agent}</h4>
                  ...
                  <div className="mt-4 w-full bg-primary-700 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full ${
                        agent.rate >= 90 ? 'bg-green-500' :
                        agent.rate >= 75 ? 'bg-yellow-500' : 'bg-red-500'
                      }`}
          ...
          {/* Usage Patterns */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">使用模式</h3>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-primary-700">
                    <th className="text-left py-3 px-4 text-sm font-medium text-primary-300">代理</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-primary-300">使用時數</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-primary-300">效率</th>
                    <th className="text-left py-3 px-4 text-sm font-medium text-primary-300">狀態</th>
                  </tr>
                </thead>
                <tbody>
                  {analyticsData.agentPerformance.usagePatterns.map((agent) => (
                    <tr key={agent.agent} className="border-b border-primary-800">
                      <td className="py-3 px-4 text-sm text-accent-50">{agent.agent}</td>
                      <td className="py-3 px-4 text-sm text-primary-300">{agent.hours}h</td>
                      <td className="py-3 px-4 text-sm text-primary-300">{agent.efficiency}%</td>
                      <td className="py-3 px-4">
                        <span className={`inline-flex px-2 py-1 text-xs rounded-full ${
                          agent.efficiency >= 80 ? 'bg-green-900 text-green-300' :
                          agent.efficiency >= 60 ? 'bg-yellow-900 text-yellow-300' :
                          'bg-red-900 text-red-300'
                        }`}>
                          {agent.efficiency >= 80 ? '優秀' : agent.efficiency >= 60 ? '良好' : '需改進'}
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      {activeMetric === 'time' && analyticsData?.timeMetrics && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <h3 className="text-sm font-medium text-primary-300">平均響應時間</h3>
              <p className="text-2xl font-bold text-accent-50">{analyticsData.timeMetrics.responseTime.average}ms</p>
            </div>
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <h3 className="text-sm font-medium text-primary-300">95th 百分位</h3>
              <p className="text-2xl font-bold text-accent-50">{analyticsData.timeMetrics.responseTime.p95}ms</p>
            </div>
            <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
              <h3 className="text-sm font-medium text-primary-300">99th 百分位</h3>
              <p className="text-2xl font-bold text-accent-50">{analyticsData.timeMetrics.responseTime.p99}ms</p>
            </div>
          </div>

          {/* Peak Hours */}
          <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
            <h3 className="text-lg font-semibold text-accent-50 mb-4">每日活動模式</h3>
