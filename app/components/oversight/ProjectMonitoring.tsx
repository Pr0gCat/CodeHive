  ...
  const filteredAndSortedProjects = projects
    .filter(project => {
      ...
      switch (filter) {
        ...
        case 'warning':
          return project.alerts.some(alert => alert.type === 'warning' || alert.type === 'error');
        case 'completed':
    ...
    .sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'activity':
          return multiplier * (new Date(a.activity.lastActivity).getTime() - new Date(b.activity.lastActivity).getTime());
        case 'progress':
          return multiplier * (a.progress.percentage - b.progress.percentage);
        case 'tokens':
          return multiplier * (a.tokenUsage.percentage - b.tokenUsage.percentage);
        case 'updated':
        default:
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {filteredAndSortedProjects.map((project) => (
          <div key={project.id} className="bg-primary-900 rounded-lg border border-primary-700 hover:border-primary-600 transition-all">
            {/* Project Header */}
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-primary-400 mt-1">
                    {project.progress.epicsCompleted} / {project.progress.totalEpics} epics
                  </p>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className={getTokenUsageColor(project.tokenUsage.percentage)}>
                  ...
                  <div className="w-full bg-primary-700 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full transition-all duration-300 ${
                        project.tokenUsage.percentage >= 90 ? 'bg-red-500' :
                        project.tokenUsage.percentage >= 75 ? 'bg-yellow-500' : 'bg-green-500'
                      }`}
                      style={{ width: `${Math.min(project.tokenUsage.percentage, 100)}%` }}
                    />
                  </div>
                  <p className="text-xs text-primary-400 mt-1">
                    {project.tokenUsage.used.toLocaleString()} / {(project.tokenUsage.used + project.tokenUsage.remaining).toLocaleString()}
                  </p>
                </div>
              </div>
            </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-accent-50 font-medium">
                      {new Date(project.activity.lastActivity).toLocaleDateString()}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
