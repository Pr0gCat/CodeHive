...
import { Code, Clock, CheckCircle, Play, Pause, RotateCcw, Star } from 'lucide-react';
...
export function DevelopmentTab({ projectId }: DevelopmentTabProps) {
  ...
  const getPhaseColor = (phase: string) => {
    switch (phase) {
      ...
      case 'REFACTOR': return 'bg-blue-100 text-blue-800';
  ...
  const getPhaseIcon = (phase: string) => {
    switch (phase) {
      ...
      case 'REFACTOR': return <RotateCcw className="h-4 w-4" />;
  ...
  if (cycles.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold text-accent-50 mb-2">無開發活動</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Live TDD Cycles */}
      <div className="bg-primary-900 rounded-lg border border-primary-700 p-6">
        <h3 className="text-lg font-semibold text-accent-50 mb-4">即時 TDD 循環</h3>
        <div className="space-y-4">
          {cycles.map((cycle) => (
            <div key={cycle.id} className="border border-primary-700 rounded-lg p-4">
              ...
              <div className="flex items-center gap-4 text-xs text-primary-400">
                ...
                {cycle.startedAt && (
                  <span>開始時間: {new Date(cycle.startedAt).toLocaleTimeString()}</span>
                )}
              </div>
            </div>
          ))}
