  ...
  const fetchActiveSprint = async () => {
    try {
      ...
      const activeResponse = await fetch(
        `/api/sprints/active?projectId=${projectId}`
      );
      ...
      if (activeData.active) {
        ...
        const planningResponse = await fetch(
          `/api/sprints?projectId=${projectId}&status=PLANNING`
        );
          ...
          setActiveSprint({
            ...
            metrics: {
              ...
              daysRemaining: Math.ceil(
                (new Date(planningData[0].endDate).getTime() - Date.now()) /
                  (1000 * 60 * 60 * 24)
              ),
            },
            _count: {
  ...
  return (
    <div className="bg-primary-700 rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        {activeSprint.status === 'ACTIVE' && (
          <div
            className={`flex items-center gap-1 ${getHealthColor(activeSprint.health)}`}
          >
            {getHealthIcon(activeSprint.health)}
            <span className="text-xs font-medium">
              {activeSprint.health === 'ON_TRACK'
                ? '正常'
                : activeSprint.health === 'AT_RISK'
                  ? '有風險'
                  : '落後'}
            </span>
          </div>
            ...
            <div>
              ...
              <div className="w-full bg-primary-600 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${activeSprint.metrics.timeElapsedPercentage}%`,
                  }}
                />
              </div>
            </div>
