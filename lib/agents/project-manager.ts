...
export class ProjectManagerAgent {
  ...
  private async generateProjectSummary(
    ...
    try {
      ...
      console.log(
        `🤖 Using Claude Code /init for intelligent project analysis: ${name}`
      );
      ...
      if (result.success && result.output) {
        ...
        if (!description) {
          ...
          if (
            ...
            content.includes('agent orchestration')
          ) {
            description = 'Multi-agent development platform';
          } else if (
            content.includes('kanban') &&
            content.includes('project management')
          ) {
  ...
  async recommendNextActions(
    ...
    if (!hasRemote) {
      recommendations.push({
        ...
        suggestedCommand:
          'Review Git repository and suggest remote setup if needed',
