...
export class ProjectManagerAgent {
  ...
  private async generateProjectSummary(
    ...
    try {
      ...
      // Use Claude Code /init to analyze the project intelligently
      console.log(`🤖 Using Claude Code /init for intelligent project analysis: ${name}`);
      ...
      if (result.success && result.output) {
        // Extract project description from CLAUDE.md content generated by /init
        const claudeMdContent = result.output.trim();
        
        // Try to extract description from various patterns in CLAUDE.md
        let description = '';
        
        // Look for project overview or description sections
        const patterns = [
          /## Project Overview\s*\n\s*([^\n]+)/i,
          /## Overview\s*\n\s*([^\n]+)/i,
          /## Description\s*\n\s*([^\n]+)/i,
        ...
        for (const pattern of patterns) {
          const match = claudeMdContent.match(pattern);
          if (match) {
            description = match[1] || match[2] || match[0];
            break;
          }
        }
        
        // If no specific pattern found, try to infer from content
        if (!description) {
          // Look for key terms to identify project type
          const content = claudeMdContent.toLowerCase();
          if (content.includes('multi-agent') || content.includes('agent orchestration')) {
            description = 'Multi-agent development platform';
          } else if (content.includes('kanban') && content.includes('project management')) {
            description = 'Project management platform';
          } else if (content.includes('e-commerce') || content.includes('shopping')) {
        ...
        console.warn(
          'Claude Code /init failed, falling back to basic description'
        );
