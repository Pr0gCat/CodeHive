...
export function initializeSampleLogs() {
  ...
  setTimeout(() => {
    projectLogger.info('system', 'application', 'CodeHive application started', {
      version: '1.0.0',
      environment: process.env.NODE_ENV || 'development'
    });
    
    projectLogger.info('system', 'database', 'Database connection established', {
      provider: 'sqlite',
      status: 'connected'
    });
...
export function addInitialProjectLogs(projectId: string, projectName: string) {
  ...
  projectLogger.info(projectId, 'project-manager', `Project "${projectName}" accessed`, {
    timestamp: new Date().toISOString(),
    action: 'access'
  });
  
  projectLogger.debug(projectId, 'file-system', `Scanning project directory structure`, {
    projectId,
    action: 'scan'
  });
